...
public class Order {
	...
	public String getAlName(int i) {
		return this.alName.get(i);
	}
	public int getAlqty(int i) {
		return this.alQty.get(i);
	}
	public int getAlSum(int i) {
		return this.alSum.get(i);
	}
	
	void control(Menu oMenu) {
		while(true) {
			System.out.println("Select a task ['c':Add Order, 'd':Delete Order, 'r':Show Orderlist,'':Exit Order");
	...
	void delete() {
		while(true) {
			...
			//지울 주문번호 입력
			//삭제
				System.out.println("enter a menu to be deleted.''Exit");
				for(int i=0; i<alName.size(); i++) {
					System.out.printf("%2d %-15s %-3d %-6d\n",i,alName.get(i),alQty.get(i),alSum.get(i));
				}
				String menuNum = this.s1.nextLine();
				if(menuNum.equals(""))break;
				int o_num = Integer.parseInt(menuNum);
				if(o_num==-1) {
					this.alName.clear();
					this.alQty.clear();
					this.alSum.clear();
